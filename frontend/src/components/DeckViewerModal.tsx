...
export default function DeckViewerModal({
  ...
  return (
    <div
      className="fixed inset-0 z-[100] bg-cyber-dark/95 backdrop-blur-md flex items-center justify-center p-4"
    ...
    >
      <div className="w-full max-w-5xl max-h-[85vh] bg-cyber-darker border-2 border-neon-magenta/40 p-6 rounded-lg shadow-[0_0_30px_rgba(255,0,255,0.15)] relative flex flex-col animate-slide-in">
        ...
        {deleteModeSupported && (
          <div className="mb-4 bg-cyber-surface/40 border border-cyber-border/40 p-3 rounded flex items-center gap-2 text-xs font-mono text-cyber-text-dim">
                      ...
                      <div className="w-full mt-1">
                        {isConfirming ? (
                          ...
                          <button
                            ...
                            onMouseEnter={() => playSE('hover')}
                            className="w-full py-1 px-3 rounded border text-[10px] uppercase font-bold tracking-wider font-mono flex items-center justify-center gap-1.5 transition-all duration-300 border-neon-red/50 text-neon-red hover:bg-neon-red/10 cursor-pointer shadow-[0_0_8px_rgba(255,0,80,0.1)]"
                          >
                            <Trash2 size={10} />
                            {t('deleteCardText')}
                          </button>
