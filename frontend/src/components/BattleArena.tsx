...
function BattleArena({
  ...
  const opponentHandCount = lastLogEntry?.cpuHandCount ?? 0;
  const playerHandCount = lastLogEntry?.playerHandCount ?? 0;
  ...
  return (
    <div className="min-h-screen bg-cyber-dark relative overflow-hidden flex flex-col justify-between p-4 select-none">
      ...
      <div className="relative z-10 max-w-7xl mx-auto w-full my-4 flex-1">
        {/* Flash Overlays (cover the whole arena) */}
        {flashState === 'cyan' && (
          <div
        ...
        {/* Turn Status Banner (shared, above the columns) */}
        <div className="my-3 w-full max-w-md mx-auto text-center z-10 relative">
          {isReplayFinished ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-stretch">
          ...
          <div className="flex flex-col gap-3 font-mono">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-neon-cyan font-bold text-sm">{playerName}</span>
            </div>

            {/* Credits & Fans */}
            {(credits !== undefined || fans !== undefined) && (
              <div className="flex gap-3 text-xs text-cyber-text-dim">
                {credits !== undefined && <span>Credits: {credits}¢</span>}
                {fans !== undefined && <span>Fans: {fans}</span>}
              </div>
            )}
            ...
            <div>
              {playerHand.length > 0 ? (
                <div className="flex flex-wrap gap-1">
                  {playerHand.map((c, i) => (
                    <CardDisplay key={i} card={c} compact />
                  ))}
                </div>
              ) : (
                <span className="text-cyber-text-dim text-xs">Hand: {playerHandCount}</span>
              )}
            </div>
            ...
            {/* Flag / Challenger Cards — Player is defender */}
            {flagHolderName === playerName ? (
              <div className="flex flex-col items-center p-3 border border-cyber-border/20 rounded-lg bg-cyber-surface/5">
                <div className="flex items-center gap-1.5 px-4 py-1.5 rounded border border-neon-cyan text-neon-cyan bg-cyan-950/10 text-glow-cyan shadow-[0_0_15px_rgba(0,240,255,0.15)] text-[10px] font-mono font-bold">
                  <Flag size={12} className="animate-pulse text-neon-cyan" />
                  <span className="uppercase">DEFENDING / あなたが支配中</span>
          ...
          <div className="flex flex-col gap-3 font-mono">
            ...
            <div className="flex items-center gap-2 justify-end">
              <span className="text-neon-magenta font-bold text-sm">{opponent}</span>
              <Cpu size={14} className="text-neon-magenta" />
            </div>

            {/* Opponent Hand Count */}
            <div className="text-right text-xs text-cyber-text-dim">
              Hand: {opponentHandCount}
            </div>

            {/* Opponent Memory Slots */}
            ...
            {/* Flag / Challenger Cards — Opponent is defender */}
            {flagHolderName === opponent ? (
              <div className="flex flex-col items-center p-3 border border-cyber-border/20 rounded-lg bg-cyber-surface/5">
                <div className="flex items-center gap-1.5 px-4 py-1.5 rounded border border-neon-magenta text-neon-magenta bg-purple-950/10 text-glow-magenta shadow-[0_0_15px_rgba(255,0,255,0.15)] text-[10px] font-mono font-bold">
                  <Flag size={12} className="animate-pulse text-neon-magenta" />
                  <span className="uppercase">DEFENDING / {opponent} が支配中</span>
                  {flagPowerValue > 0 && <span className="ml-2 font-black border-l border-cyber-border/40 pl-2 text-white">{flagPowerValue} POW</span>}
                </div>
                <div className="mt-3 flex items-center justify-center min-h-[140px]">
                  {currentFlagCard ? (
              ...
              // Opponent is attacker (challenger)
              <div className="flex flex-col items-center p-3 border border-cyber-border/20 rounded-lg bg-cyber-surface/5">
                <div className="text-[9px] font-mono text-cyber-text-dim/60 uppercase tracking-widest mb-2 flex items-center gap-2">
                  <span>ACTIVE CHALLENGE AUGMENTATIONS / 挑戦者めくりカード</span>
                  {challengerPower > 0 && (
            ...
            {/* Opponent Draw Button (only when opponent is NPC and it's opponent's draw turn) */}
            {!isReplayFinished && isOpponentDrawTurn && opponentIsNPC && (
              <div className="w-full mt-4 z-10 flex flex-col items-center gap-2">
                <button
                  disabled={isLiveMode && (battleSession.requiredAction !== 'DRAW')}
                  onClick={() => {
                    playSE('click');
                    if (isLiveMode) {
                      onStep();
                  ...
                  className={`w-full max-w-xs py-2.5 px-5 rounded border-2 font-mono font-bold text-xs uppercase tracking-widest cursor-pointer transition-all duration-150 transform active:scale-95 shadow-md flex items-center justify-center gap-2 border-neon-magenta text-neon-magenta bg-purple-950/20 hover:bg-purple-950/40 text-glow-magenta shadow-[0_0_15px_rgba(255,0,255,0.2)]`}
                >
                  <Play size={14} />
        ...
        {/* Interactive Choice Overlay (full screen over the whole arena) */}
